...
map <- leaflet(well_locations) %>%
  ...
  addPolylines(data = ws3streams$StrType, fillColor = streams_col(ws3streams$StrType), group = "Streams") %>%
  addCircleMarkers(lng = well_locations$POINT_X, lat = well_locations$POINT_Y,
             color = "Black", popup = paste("Well ID:", well_labels$Well, "X Coordinate:", well_labels$POINT_X)) %>%
