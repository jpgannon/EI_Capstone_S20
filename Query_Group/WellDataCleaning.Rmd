...
waterChemData <- allWaterChemData %>% mutate(Date = ymd(Date))
...
allWeirData <- rbind(weirEarlyData, weirLaterData)
...
#filter out unnecessary columns and plots
slopeAspElevData <- allSlopeAspData %>% mutate(date = mdy(SAMPLE_DATE)) %>% select(-c(SAMPLE_DATE, DATE_NOTE))
```
