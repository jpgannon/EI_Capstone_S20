...
#select input that allows the user to select which HPU units they want to see
selectInput("hpu", "Select desired HPU:", choices = hpu_data, multiple = TRUE)
...
observeEvent(input$slope, {updateSelectInput(session, input = "select_wells", label = NULL, choices = getWellList(input$slope[1], input$slope[2], input$aspect[1], input$aspect[2]), selected = NULL)})

observeEvent(input$aspect, {updateSelectInput(session, input = "select_wells", label = NULL, choices = getWellList(input$slope[1], input$slope[2],input$aspect[1], input$aspect[2]), selected = NULL)})

#observeEvent(input$hpu, {updateSelectInput(session, input = "select_wells", label = NULL, choices = })
...
observeEvent(input$selectAllWellsBox, 
             {updateSelectInput(session, 
                                ...
                                selected =if(input$selectAllWellsBox == TRUE){c(
                                  getWellList(input$slope[1], input$slope[2], input$aspect[1], input$aspect[2]))
                                  }
                                )
               }
             )
...
#plots selected wells' well water level data
  ggplot(data = selected_well_data, mapping = aes(x = date, y = level, color = Well)) + 
    ...
    coord_cartesian(xlim = as.POSIXct(final_dates$x, origin = "1970-01-01"), expand = FALSE)+
      theme(text = element_text(size=input$mag))  #add this line to all graphs in order to change the font sizes 
})
```
...
getWellList <- function(minValSlope, maxValSlope, minValAsp, maxValAsp) { 
    ...
    #hpuWells <- hpu_data %>% filter(HPU = hpuVal)
    return(t(wells))
  }
```
