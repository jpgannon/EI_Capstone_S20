...
renderPlot({
  ...
  #takes input from the Prange slider
  precip_trim <- precip_trim %>% 
    mutate(Highlight = ifelse(Value >= input$Prange[1] & Value <= input$Prange[2], 
                              Value, NA))
  ...
  ggplot(data = precip_trim, aes(fill = Type, y = Value, x = DATE)) +
    ...
    annotation_custom(grob) + theme(legend.position = "bottom")
})
```
...
renderPlot({
  ...
  if(input$log) d_plot <- d_plot + scale_y_log10() 
  d_plot
})
```
...
#https://www.w3schools.com/tags/default.asp this link will help you with html inside 
