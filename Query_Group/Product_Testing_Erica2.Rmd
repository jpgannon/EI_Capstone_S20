...
observeEvent(input$slope, {print(getWellList(input$slope[1], input$slope[2]))})
observeEvent(input$slope, {updateSelectInput(session, input = "select_wells", label = NULL, choices = getWellList(input$slope[1], input$slope[2]), selected = NULL)})
...
```{r}
getWellList <- function(maxVal, minVal) { 
    wells <- slope_asp_data %>% filter(Slope >= input$slope[1] && Slope <= input$slope[2]) %>% select(Well)
    return(c(wells))
  }
```
