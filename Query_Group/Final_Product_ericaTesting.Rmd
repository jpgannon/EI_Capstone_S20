...
well_info_data <- read_csv("w3well_locations.txt")
```
...
sliderInput("slope", "Select a slope range", min = 3.96, max = 28.4,
            value = c(3.963356, 28.42197), ticks = TRUE, dragRange = TRUE)

# make a slider widget for aspect
sliderInput("aspect", "Select an aspect range", min = 0.05, max = 344.6,
            value = c(0.05, 344.6), ticks = TRUE, dragRange = TRUE)

#make a slider widget for pipe height
sliderInput("pipeHt", "Select a pipe height range",
            min = min(well_info_data$PipeHt, na.rm = TRUE),
            max = max(well_info_data$PipeHt, na.rm = TRUE), 
            value = c(min(well_data$PipeHt), max(well_data$PipeHt)),
            ticks = TRUE, 
            dragRange = TRUE)
...
observeEvent(input$slope, {
  updateSelectInput(session, input = "select_wells", label = NULL, 
                    choices = getWellList(input$slope[1], input$slope[2], 
                                          ...
                                          input$hpu, input$pipeHt[1], input$pipeHt[2]), selected = NULL)})
...
getWellList <- function(minValSlope, maxValSlope, minValAsp, maxValAsp, hpuValsList, minValPipe, maxValPipe) { 
    ...
    pipeWells <- well_info_data %>% 
      filter(between(PipeHt, minValPipe, maxValPipe)) %>% 
      select(Well)
    ...
    combWells <- intersect(tempIntersect, pipeWells)
