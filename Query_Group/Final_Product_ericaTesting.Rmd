...
well_info_data <- read_csv("extra_well_data.csv")
...
sliderInput("twi", "Select a TWI range", 
            min = min(well_info_data$TWI), max = max(well_info_data$TWI), 
            value = c( min(well_info_data$TWI),max(well_info_data$TWI)),
            ticks = TRUE, dragRange = TRUE)

#make a slider input for UAA
sliderInput("uaa", "Select a TWI range", 
            min = min(well_info_data$UAA_Weighted), max = max(well_info_data$UAA_Weighted), 
...
#variable that holds the parameters to be passed to the getWellsList function
getWellsParams <- reactiveValues(x = c(input$slope[1], input$slope[2], 
                                         input$aspect[1], input$aspect[2], 
                                          input$hpu, input$wellDepth, input$twi[1], input$twi[2], input$uaa[1], input$uaa[2]))
...
observeEvent(input$slope, {
  updateSelectInput(session, input = "select_wells", label = NULL, 
                    choices = getWellList(getWellsParams), selected = NULL)})
...
observeEvent(input$hpu, {
  updateSelectInput(session, input = "select_wells", label = NULL, 
                    choices = getWellList(getWellsParams))})
...
getWellList <- function(getWellsParams) { 
    minValSlope = getWellsParams$x[1]
    maxValSlope = getWellsParams$x[2]
    minValAsp = getWellsParams$x[3]
    maxValAsp = getWellsParams$x[4]
    hpuValsList = getWellsParams$x[5]
    depthValsList = getWellsParams$x[6]
    minTwi = getWellsParams$x[7]
    maxTwi = getWellsParams$x[8]
    minUaa = getWellsParams$x[9]
    maxUaa = getWellsParams$x[10]
