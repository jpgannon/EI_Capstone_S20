...
dates <- reactiveValues(
  range = as.POSIXct(c(start = "2010-07-01", end = "2018-01-10"))
  )

well_vals <- reactiveValues(
  slope = c(3.963356, 28.42197), asp = c(0.055854, 344.6115), 
  twi = c(5.092, 13.306), uaa = c(0, 0.631), wellDepths = unique(well_info_data$deep_well), hpu = unique(hpu_data$HPU)
  )
...
getWellList <- function() { 
  ...
  depthWells <- hpu_data %>% subset(deep_well %in% well_vals$wellDepths) %>% select(Well)
  ...
  #makes a second temporary list of wells to combine the first temporary list with the wellInfoWells list
...
observeEvent(input$date_range, {
  dates$range <- as.POSIXct(c(input$date_range[1], input$date_range[2]))
  })
...
observeEvent(input$twa, {
  well_vals$twa <- c(input$twa[1], input$twa[2])
...
observeEvent(input$wellDepth, {
  well_vals$wellDepth <- input$wellDepth
...
renderPlot({
  ...
  precip_trim <- precip_data %>% 
    filter(between(as.POSIXct(DATE), dates$range[1], dates$range[2])) %>% 
...
output$water_level <- renderPlot({
  ...
  selected_well_data <- well_data %>% filter(Well == input$select_wells) %>% 
    filter(between(date, dates$range[1], dates$range[2]))
