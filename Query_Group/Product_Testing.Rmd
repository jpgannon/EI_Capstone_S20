...
renderPlot({
  ...
  list_names <- unlist(input$select)
  selected_data <- filter(well_data, Well == list_names)
  ...
  ggplot(data = selected_data, mapping = aes(x = date, y = level)) +
    geom_line() +
    scale_y_reverse() +
    ylab("Water Table Depth (cm)") +
    xlab("Date") + 
    # makes the auto zoom work, but manual input date doesnt work
    # coord_cartesian(xlim = as.Date(ranges$x, origin = "1970-01-01"), 
    #               expand = FALSE) + 
    # The manual input date
